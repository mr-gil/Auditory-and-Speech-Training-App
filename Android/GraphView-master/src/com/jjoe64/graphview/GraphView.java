...
abstract public class GraphView extends LinearLayout {
	...
	//added by tom from source of formatLabel()
	@Deprecated
	protected String formatLabel2(double value, boolean isValueX) {
		if (customLabelFormatter != null) {
			String label = customLabelFormatter.formatLabel(value, isValueX);
			if (label != null) {
				//return String.valueOf(Math.round(Float.valueOf(label)))+"%";
				return label + "%";
			}
		}
		int i = isValueX ? 1 : 0;
		...
		//return String.valueOf(Math.round(Float.valueOf(numberformatter[i].format(value)))) + "%";
		return numberformatter[i].format(value) +  "%";
	}
	...
	synchronized private String[] generateVerlabels(float graphheight) {
		...
		for (int i=0; i<=numLabels; i++) {
			labels[numLabels-i] = formatLabel2(min + ((max-min)*i/numLabels), false);
		}
		return labels;
	}
